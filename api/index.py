...
def extract_protobuf_info(protobuf_data):
    ...
    try:
        # Convert protobuf to dictionary for easier access
        json_str = MessageToJson(protobuf_data)
        data = json.loads(json_str)
        
        # Extract region and level from the nested structure
        # Based on the structure you provided: "1": {"data": {"5": {"data": ""}, "6": {"data": 0}}}
        region = ""
        level = 0
        
        if '1' in data and 'data' in data['1']:
            inner_data = data['1']['data']
            if '5' in inner_data and 'data' in inner_data['5']:
                region = inner_data['5']['data']
            if '6' in inner_data and 'data' in inner_data['6']:
                level = inner_data['6']['data']
...
def handle_requests():
    ...
    try:
        ...
        def process_request():
            ...
            # Extract region and level from protobuf
