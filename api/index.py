...
@app.route("/likeinfo", methods=["GET"])
def like_api():
    uid = request.args.get("uid")
    ...
    try:
        ...
        before = make_request(encrypted_uid, server, token)
        if before is None:
        ...
        # Send likes (using existing function)
        asyncio.run(send_multiple_requests(uid, server, like_url))

        # Get player info after
        after = make_request(encrypted_uid, server, token)
        if after is None:
        ...
        app.logger.error(f"Error in likeinfo endpoint: {e}")
        return jsonify({"error": str(e)}), 500
