...
# Ensure the script can find the `my_api` module
...
async def send_multiple_requests(uid, server_name, url):
    try:
        ...
        if tokens is None:
            ...
            return None
        random_indices = random.sample(range(len(tokens)), min(100, len(tokens)))  # Limit to available tokens
